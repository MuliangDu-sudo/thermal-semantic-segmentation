...
class Canny(nn.Module):
    def __init__(self, device, batch_size, threshold=10.0, num_channel=1):
    ...
    def forward(self, img):
        if self.num_channel == 3:
            ...
            raise ValueError('Channel number does not fit.')
        ...
        pixel_range = torch.cat(self.batch_size*[torch.FloatTensor([range(pixel_count)])]).view(-1).to(self.device)

        # pixel_range = torch.cuda.FloatTensor([range(pixel_count)])
        indices = (inidices_positive.view(-1).data * pixel_count + pixel_range).squeeze()
        channel_select_filtered_positive = all_filtered.view(-1)[indices.long()].view(self.batch_size, height, width)
        indices = (inidices_negative.view(-1).data * pixel_count + pixel_range).squeeze()
        channel_select_filtered_negative = all_filtered.view(-1)[indices.long()].view(self.batch_size, height, width)
