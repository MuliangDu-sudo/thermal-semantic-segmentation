...
class ResNet101(nn.Module):
    def __init__(self, block, layers, num_classes, BatchNorm, bn_clr=False, num_channels=3):
        ...
        self.conv1 = nn.Conv2d(self.num_channels, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def freeze_bn_func(m):
    if m.__class__.__name__.find('BatchNorm') != -1 or isinstance(m, nn.BatchNorm2d):
            # or isinstance(m, SynchronizedBatchNorm2d)\
...
def Deeplab(BatchNorm, num_classes=21, num_channels=3, freeze_bn=False, restore_from=None, initialization=None, bn_clr=False):
    ...
    # else:
    #     pretrain_dict = torch.load(initialization)['sem_net_state_dict']
    # model_dict = {}
    ...
    if restore_from is not None: 
        ...
        model.load_state_dict(checkpoint["sem_net_state_dict"])
